...
curves = [secp256k1, bn254, bw6761, bls12377]

for curve in curves:
    print("- Test on", curve["name"])
    E = curve["curve"]
    K.<j> = curve["ring"]
    r = curve["r"]
    l = curve["l"]
    ...
    # constants per curve (we can hardcode them)
    r0, r1 = find_basis(0,r,l)[0]
    ...
    for i in range(100):
        ...
        assert(u0*P + u1*phi(E, P) - v0*Q - v1*phi(E, Q) == E(0))
        print("proving [{}]P == Q is equivalent to proving:".format(s))
        print("[{}]P + [{}]phi(P) - [{}]Q - [{}]phi(Q) == 0".format(u0, u1, v0, v1))
